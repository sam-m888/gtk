  ...
  if (priv->wrap_width > 0 || !priv->cell_view)
    {
      ...
      gtk_widget_get_allocation (attach_widget, &attach_allocation);
      gtk_css_gadget_get_border_allocation (priv->gadget, &border_allocation, NULL);
      gtk_css_gadget_get_content_allocation (priv->gadget, &content_allocation, NULL);

      attach_allocation.y -= content_allocation.y - border_allocation.y;
      attach_allocation.height += 2 * (content_allocation.y - border_allocation.y);

      gdk_attach_params_set_attach_rect (params, &attach_allocation, gtk_widget_get_window (attach_widget));
  ...
  else
    {
      ...
      for (offset_y = 0; i && i->data != active_widget; i = i->next)
        {
      ...
      gdk_attach_params_set_anchors (params, GDK_ATTACH_LEFT, GDK_ATTACH_TOP_LEFT);
      gdk_attach_params_set_attach_hints (params, GDK_ATTACH_NO_HINTS);
